...
./live-spec.html has a friend, lspec.py, the companion tool (verbs listed in `python3 lspec.py start`'s output); the hooks in ./hooks run `check --staged` on every commit (pre-commit: structure + seal gate; commit-msg: review + vocabulary gates), and `check --clean` is the post-commit completion check.
